"""
TODO: Code for automatic brain extraction via T1w image
...
mask_weight = np.zeros((128, 128, 128))
...
class SwinUNet_Joint(nn.Module):
    def __init__(self):
        super().__init__()
        # self.Boundary = SwinUNETR(img_size=(128, 128, 128), in_channels=2, out_channels=4, depths=(2, 4, 2, 2), use_v2=True)
        self.Tissue = SwinUNETR(img_size=(128, 128, 128), in_channels=2, out_channels=4, depths=(2, 4, 4, 2), use_v2=True)
        self.Parc = SwinUNETR(img_size=(128, 128, 128), in_channels=5, out_channels=107, depths=(2, 4, 4, 2), use_v2=True)

    def forward(self, x):
        tissue = self.Tissue(x)
        region_input = torch.cat([x, tissue[:, 1:, :, :, :]], dim=1)
        parc = self.Parc(region_input)
...
def _get_pred(args, model, img):
    ...
    pred_rec = torch.zeros((args.num_classes+1, W, H, D))
    pred_rec_tissue = torch.zeros((4, W, H, D))

    for start_pos in pos:
        patch = img[:, :, start_pos[0]:start_pos[0] + args.crop_size[0], start_pos[1]:start_pos[1] + args.crop_size[1],
                start_pos[2]:start_pos[2] + args.crop_size[2]]

        model_out_tissue, model_out_parc = model(patch)

        model_out_parc = model_out_parc.cpu().detach()
        pred_rec[:, start_pos[0]:start_pos[0] + batch_size[0], start_pos[1]:start_pos[1] + batch_size[1], start_pos[2]:start_pos[2] + batch_size[2]] += model_out_parc[0, :, :, :, :] * mask_weight

        model_out_tissue = model_out_tissue.cpu().detach()
        pred_rec_tissue[:, start_pos[0]:start_pos[0] + batch_size[0], start_pos[1]:start_pos[1] + batch_size[1], start_pos[2]:start_pos[2] + batch_size[2]] += model_out_tissue[0, :, :, :, :] * mask_weight

    return pred_rec_tissue, pred_rec
...
def get_pred(args, model, source, target, item):
    ...
    if args.norm_spacing == 1:
        ...
        pred_tissue = nn.functional.interpolate(pred_tissue, size=(shape[0], shape[1], shape[2]), mode='trilinear',
                                                antialias=False)
        pred_tissue = pred_tissue.squeeze(0)

        pred_parc = pred_parc.unsqueeze(0)
        pred_parc = nn.functional.interpolate(pred_parc, size=(shape[0], shape[1], shape[2]), mode='trilinear',
                                              antialias=False)
...
def _get_pred_parallel(args, model, source, target, item):
    target_tissue_path = os.path.join(target, item, 'tissue.nii.gz')
    target_dk_path = os.path.join(target, item, 'dk-struct.nii.gz')
    pred_tissue, pred_dk = get_pred(args, model, source, target, item)
    ants.image_write(pred_tissue, target_tissue_path)
    ants.image_write(pred_dk, target_dk_path)
