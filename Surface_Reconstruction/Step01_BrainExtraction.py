...
def _model_init(args, model_path):
    ...
    model = SwinTransformerSys3D(in_chans=1, num_classes=2)
...
def _normalize_z_score_CT(data, clip=True):
    '''
    funtions to normalize data to standard distribution using (data - data.mean()) / data.std()
    :param data: numpy array
    :param clip: whether using upper and lower clip
...
def _get_pred(args, model, img):
    ...
    pred_rec_s = torch.zeros((2, W, H, D))
    ...
    for start_pos in pos:
        ...
        pred_rec_s[:, start_pos[0]:start_pos[0] + args.crop_size[0], start_pos[1]:start_pos[1] + args.crop_size[1],
        start_pos[2]:start_pos[2] + args.crop_size[2]] += model_out_s[0, :, :, :, :] * mask_weight
...
def _inference(args, model, img_path):
    origin, spacing, direction, img = _ants_img_info(img_path)
    if args.modality == 'CT':
        img = _normalize_z_score_CT(img)
    else:
...
def get_pred(args, model, source, target, item):
    ...
    ants_img_pred = ants.from_numpy(pred, origin, spacing, direction)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str,
                        default='/public_bme/data/sMRI_Process_Tool/BET.pth.gz',
                        help='Pretrained model path')
    parser.add_argument('--crop_size', type=tuple, default=(128, 128, 128), help='patch size')
    parser.add_argument('--modality', type=str, default='T1', help='patch size')
    ...
    # Single inference will occupy around 10GB GPU memory
    pool_num = 3
    pool = Pool(pool_num)
    ...
    for item in file_list:
        ...
        pool.apply_async(get_pred_parallel, args=(), kwds=kwargs, callback=call_fun, error_callback=error_back)
